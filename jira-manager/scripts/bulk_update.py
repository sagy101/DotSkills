...
def _print_dry_run_summary(
    ...
    for i in issues[:20]:
        ...
        print(
            f"{i['key']:<10} {fields_data.get('status', {}).get('name', '?'):<15} {fields_data.get('summary', '')[:50]}"
        )
...
def _update_single_issue(
    ...
    if status and not handle_status_transition(client, key, status, config, warn_only=True):
        ok = False
    if sprint_id:
...
def main() -> None:
    ...
    parser.add_argument(
        "--confirm", action="store_true", help="Execute the updates (default is dry-run)"
    )
    ...
    if not args.confirm:
        _print_dry_run_summary(
            issues, fields, effective_status, effective_sprint, config, args.attachment
        )
        print("\nTo execute these changes, run again with --confirm")
        return

    # 4. Execute Updates
    execute_bulk_updates(
        client, issues, fields, effective_status, effective_sprint, args.attachment, config
    )
