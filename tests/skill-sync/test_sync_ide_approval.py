...
class TestGeminiPolicyGeneration:
    ...
    def test_uses_correct_gemini_schema(self, read_patterns):
        """Must use Gemini CLI policy engine field names, not arbitrary ones."""
        content = _generate_gemini_policy(read_patterns)
        # Must NOT have 'tool = "shell"' (old incorrect format)
        assert 'tool = "shell"' not in content
        # Must NOT have 'description =' (not a valid TOML rule field)
        assert 'description =' not in content
        # Must have priority (required by Gemini CLI)
    ...
    def test_each_skill_mentioned_in_comments(self, read_patterns):
        content = _generate_gemini_policy(read_patterns)
        for entry in read_patterns:
...
class TestGeminiInstall:
    ...
    def test_idempotent_preserves_content(self, source_with_patterns, gemini_detected, tmp_path):
        """After 2nd run, file content must be identical to 1st run."""
        update_gemini_approval(source_with_patterns, gemini_detected, dry_run=False)
        policy = tmp_path / "user" / ".gemini" / "policies" / _GEMINI_POLICY_FILE
        content_after_first = policy.read_text()
        update_gemini_approval(source_with_patterns, gemini_detected, dry_run=False)
        content_after_second = policy.read_text()
        assert content_after_first == content_after_second

    def test_does_not_overwrite_other_policy_files(self, source_with_patterns, gemini_detected, tmp_path):
        """Other policy files in the same directory must not be touched."""
        policy_dir = tmp_path / "user" / ".gemini" / "policies"
        policy_dir.mkdir(parents=True)
        custom = policy_dir / "my-custom-rules.toml"
        custom.write_text('[[rule]]\ncommandRegex = "npm test"\ndecision = "allow"\npriority = 50\n')
        update_gemini_approval(source_with_patterns, gemini_detected, dry_run=False)
        assert custom.read_text().startswith("[[rule]]")
        assert "npm test" in custom.read_text()
...
class TestWindsurfSettingsMerge:
    def test_creates_new_settings(self, tmp_path):
        ...
        assert sorted(data["windsurf.cascadeCommandsAllowList"]) == ["cmd1", "cmd2"]

    def test_uses_correct_windsurf_key(self, tmp_path):
        """Key must be windsurf.cascadeCommandsAllowList (with prefix), not bare."""
        settings = tmp_path / "settings.json"
        _update_windsurf_settings(settings, {"cmd1"}, dry_run=False)
        data = json.loads(settings.read_text())
        assert "windsurf.cascadeCommandsAllowList" in data
        assert "cascadeCommandsAllowList" not in data  # bare key should NOT exist

    def test_preserves_existing_entries(self, tmp_path):
        settings = tmp_path / "settings.json"
        settings.write_text(json.dumps({"windsurf.cascadeCommandsAllowList": ["existing_cmd"]}))
        _update_windsurf_settings(settings, {"new_cmd"}, dry_run=False)
        data = json.loads(settings.read_text())
        assert "existing_cmd" in data["windsurf.cascadeCommandsAllowList"]
        assert "new_cmd" in data["windsurf.cascadeCommandsAllowList"]
    ...
    def test_dedup_no_duplicates(self, tmp_path):
        settings = tmp_path / "settings.json"
        settings.write_text(json.dumps({"windsurf.cascadeCommandsAllowList": ["cmd1"]}))
        result = _update_windsurf_settings(settings, {"cmd1"}, dry_run=False)
        assert result is False
        data = json.loads(settings.read_text())
        assert data["windsurf.cascadeCommandsAllowList"].count("cmd1") == 1
    ...
    def test_handles_corrupt_json(self, tmp_path):
        ...
        assert result is True
        data = json.loads(settings.read_text())
        assert "cmd1" in data["windsurf.cascadeCommandsAllowList"]
...
class TestWindsurfInstallIntegration:
    ...
    def test_installs_prefixes(self, source_with_patterns, windsurf_detected, tmp_path):
        ...
        data = json.loads(settings.read_text())
        assert len(data["windsurf.cascadeCommandsAllowList"]) > 0
...
class TestCursorInfo:
    def test_prints_info(self, capsys):
        ...
        assert "YOLO" in output or "allowlist" in output
