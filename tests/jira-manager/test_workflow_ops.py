...
def _make_transitions(items: list[tuple[int, str, str]]) -> list[dict[str, object]]:
    """Build transition list: [(id, name, to_status_name), ...]"""
    return [
        {"id": str(tid), "name": tname, "to": {"name": to_name}} for tid, tname, to_name in items
    ]


def _make_config(status_values: dict[str, dict[str, str]] | None = None) -> mock.MagicMock:
    config = mock.MagicMock()
    if status_values:
        config.field_catalog = {"status": {"values": status_values}}
    else:
...
class TestResolveTransition:
    ...
    def test_match_via_catalog_resolution(self):
        ...
        config = _make_config(status_values={"completed": {"id": "10", "name": "Done"}})
...
class TestHandleStatusTransition:
    ...
    def test_api_failure_warn_only_returns_false(self):
        ...
        result = handle_status_transition(
            client, "T-1", "In Progress", _make_config(), warn_only=True
        )
        assert result is False
    ...
    def test_available_statuses_in_error_message(self, capsys: pytest.CaptureFixture[str]) -> None:
        client = mock.MagicMock()
        client.get_transitions.return_value = _make_transitions(
            [
                (1, "Start", "In Progress"),
